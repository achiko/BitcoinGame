...
var mainBtcAdress = "n47ZYEJCdPXbu1ekNM6NKFxDFVoY2XJL5L";
...
exports.PlaceBet = function(db, Models, client, SessionID, currentcoefs, BetObj, BetData, callback) {
		...
		async.series([	    
					...
					function(callback)
					{
							client.cmd('getaccount', _PlayerBTCAdress, function (err, acc) {
				 ...
				 function(callback)
				 {
			  		 client.cmd('sendfrom', _PlayerWalletAccount, mainBtcAdress, BetObj.amt, 0,  function (err, tid) {
				 ...
				 function(callback)
				 {
				 		...
				 		client.cmd('getaccount', mainBtcAdress, function (err, acc) {
			          		...
			          		if (err)  { 		         					
			        						...
			        						logger.error('5. Error Get Player Account From Bticoin Wallet %s', util.inspect(err));                   		
			        						return callback(err, 205);  
		          			}						

			          		logger.info('5. Get Player Account From Bticoin Wallet: ', acc);
			          		logger.log('5. Get Player Account From Bticoin Wallet: ',  acc);
			          		
			          		_MainWalletAccount = acc;
			          		callback(null, 105);
							});
				 },
				 ...
				 function(callback)
				 {	
				 			client.cmd('getbalance', _MainWalletAccount,  function (err, balance) {
				...
				function(callback)
				{
						...
						client.cmd('sendfrom', _MainWalletAccount, _PlayerBTCAdress, _SlipWinAmount, 0 , function (err, tid) {
				...
				function(callback)
				{
						...
						logger.log('Slip %s', util.inspect(slip));                   		
						...
						slip.save(function(err, doc){		
      			       ...
      			       logger.info('10. Insert Bet Into Database OK" ', 110);									 
									 ...
									 logger.info('Date Diff: ', datediff);
